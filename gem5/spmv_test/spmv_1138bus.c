...
int main()
{
	...
	fp=freopen("./data/1138_bus/info.txt","r",stdin);
	scanf("%d %d %d",&NNZ,&M,&N);
	fclose(fp);

	fp=freopen("./data/1138_bus/nnz.txt","r",stdin);
	/*while(scanf("%lf",&temp)){
		vals[cnt++]=temp;
	}*/
	for(int i=1;i<=NNZ;i++){
	...
	fp=freopen("./data/1138_bus/row.txt","r",stdin);
	/*while(scanf("%d",&temp1)){
		row_ptr[cnt++]=temp1;
	}*/
	for(int i=1;i<=M+1;i++){
	...
	fp=freopen("./data/1138_bus/col.txt","r",stdin);
	/*while(scanf("%d",&temp1)){
		col_indx[cnt++]=temp1;
	}*/
	for(int i=1;i<=NNZ;i++){
