...
cd ${SH_ROUTE};nohup time bash sim_generate_sim_weight.sh ${NAME} $2 > ${REC_ROUTE}/${SE_NAME}/${SE_NAME}_sim_weight.txt 
cd ${SH_ROUTE};nohup time bash sim_generate_checkpoint.sh ${NAME} $2 > ${REC_ROUTE}/${SE_NAME}/${SE_NAME}_checkpoint.txt 
times=$(ls ${GEM5_PATH}/${SE_OUT_DIR_CHECKPOINT} | wc -l)
rm -r ${REC_ROUTE}/${SE_NAME}/${SE_NAME}_O3
mkdir ${REC_ROUTE}/${SE_NAME}/${SE_NAME}_O3
for ((i=1;i<=times;i++))
do cd ${SH_ROUTE};nohup time bash sim_O3.sh $i ${NAME} $2 > ${REC_ROUTE}/${SE_NAME}/${SE_NAME}_O3/$i.txt
done
